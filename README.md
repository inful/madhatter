...
- **Calendar subscriptions**: Personal ICS calendar URLs for any calendar app. Each member's per-user feed renders their HAT-day assignments as VEVENTs, their own approved WFH days as separate all-day VEVENTs titled `<member> - WFH`, and **team birthday events** as `🎂 <member>'s birthday` all-day VEVENTs with `RRULE:FREQ=YEARLY` so they recur every year without an annual refresh. Admin-marked WFH days carry a "(marked by admin)" note in the event description. Only approved WFH rows surface on the calendar — pending, denied, and withdrawn rows do not. The event text and HTML are overridable per event kind via the standard template-override env vars (`CALENDAR_TEMPLATE_*`, `LEAVE_TEMPLATE_*`, `HOLIDAY_TEMPLATE_*`, `WFH_TEMPLATE_*`, `BIRTHDAY_TEMPLATE_*`). HAT days that have been covered (a `cover` row with `original_assignment_id` pointing at the original) are suppressed from both the per-member feed and the "others" feed so subscribers only see who is actually on duty that day; the cover renders with its `(COVER)` suffix so the truth is still visible. Birthdays are surfaced regardless of who the subscription belongs to (i.e. a member sees their teammates' birthdays on their own feed) — set `CALENDAR_BIRTHDAYS_ENABLED=false` to disable the feature entirely without touching the other event kinds.
...
- **No-one-WFH day marker**: When today has zero WFH rows and at least one person at work, the dashboard surfaces an informational **"No one is WFH today"** banner above the status card and a 🏢 **All in** chip in the schedule matrix column header (plus a mobile variant on the day card). The marker piggybacks on the same business-day gate as the full-team-on-site celebration but is orthogonal — a day where some members are on leave but everyone at work is in the office is a "no one is WFH" day without being a "full team" day. The two markers are mutually exclusive in the dashboard banner (the louder gold full-team celebration wins) but the matrix column header reads directly from the schedule data, so the no-WFH chip can still appear on a future day that satisfies the matrix condition.
- **Birthday celebrations** ([issue #60](https://github.com/inful/madhatter/issues/60)): Admins can record an optional birthdate for each team member from the team-edit form. When at least one active member has a birthday in the next 7 days (today inclusive), the dashboard renders a celebratory **🎂 birthday banner** above the status card with three copy branches: "X's birthday today!", "X's birthday this week" for a single member, or "Birthdays this week" listing multiple names. The banner is suppressed on weekends and holidays, mirroring the existing celebratory banners. On the exact day of a member's birthday, the dashboard also fires a one-time **birthday confetti blast** from the top corners — pink + gold particles interleaved with 🎂 cake and 💗 heart shapes baked into confetti shapes via `confetti.shapeFromText`. The blast is distinct from the full-team confetti burst (different origin, smaller particle count, distinct shape set) so the user can tell them apart at a glance; both can fire on the same day. The team list page shows a small inline 🎂 badge next to each member's name with a tooltip showing the date, so the team sees who has a birthday on file. Birthdates are stored as DATE columns (year preserved for admin auditing) but the dashboard banner copy surfaces only MM-DD — the year is treated as private internal data. Members who haven't shared a birthdate are excluded from both surfaces. The blast respects `BIRTHDAY_CONFETTI_ENABLED` (default `true`) and the OS-level `prefers-reduced-motion: reduce` preference.
- **Fun effects on the dashboard** ([issue #59](https://github.com/inful/madhatter/issues/59)): When the entire team is on-site on a business day, a celebratory confetti burst fires from both sides of the page using the vendored [canvas-confetti](https://github.com/catdad/canvas-confetti) library (ISC-licensed, served from `/static/` so the strict `script-src 'self'` CSP stays tight). Every December, a 45-second gently-falling snow storm runs across the dashboard using the library's particle physics. On the autumnal equinox (September 23) with the full team on-site, a 45-second storm of falling leaf emojis (🌿 herb, 🍁 maple, 🍂 fallen, 🍃 wind-fluttering) drifts down the page using `confetti.shapeFromText` to rasterise the emojis into particle sprites. All three effects respect `prefers-reduced-motion: reduce` — confetti uses the library's built-in `disableForReducedMotion` flag, snow and leaves short-circuit the whole requestAnimationFrame loop. Each effect has its own operator gate (`CONFETTI_ENABLED`, `SNOW_ENABLED`, `LEAVES_ENABLED`) so a deployment that finds any celebration distracting can disable it independently without touching the others.
### User Interface
