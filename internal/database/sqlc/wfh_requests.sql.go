// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.30.0
// source: wfh_requests.sql

package sqlc

import (
	"context"
	"database/sql"
	"time"
)

const countApprovedWFHByDate = `-- name: CountApprovedWFHByDate :one
SELECT COUNT(*) AS count
FROM wfh_requests
WHERE date = ? AND status = 'approved'
`

func (q *Queries) CountApprovedWFHByDate(ctx context.Context, date time.Time) (int64, error) {
	row := q.db.QueryRowContext(ctx, countApprovedWFHByDate, date)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const createWFHRequest = `-- name: CreateWFHRequest :execresult
INSERT INTO wfh_requests (id, member_id, date, status)
VALUES (?, ?, ?, 'pending')
`

type CreateWFHRequestParams struct {
	ID       string    `json:"id"`
	MemberID string    `json:"member_id"`
	Date     time.Time `json:"date"`
}

func (q *Queries) CreateWFHRequest(ctx context.Context, arg CreateWFHRequestParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, createWFHRequest, arg.ID, arg.MemberID, arg.Date)
}

const deleteWFHRequest = `-- name: DeleteWFHRequest :exec
DELETE FROM wfh_requests
WHERE id = ?
`

func (q *Queries) DeleteWFHRequest(ctx context.Context, id string) error {
	_, err := q.db.ExecContext(ctx, deleteWFHRequest, id)
	return err
}

const getAllWFHRequests = `-- name: GetAllWFHRequests :many
SELECT id, member_id, date, status, created_at, settled_at, withdrawn_by, withdrawn_at
FROM wfh_requests
ORDER BY date DESC, created_at DESC
`

func (q *Queries) GetAllWFHRequests(ctx context.Context) ([]WfhRequest, error) {
	rows, err := q.db.QueryContext(ctx, getAllWFHRequests)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []WfhRequest{}
	for rows.Next() {
		var i WfhRequest
		if err := rows.Scan(
			&i.ID,
			&i.MemberID,
			&i.Date,
			&i.Status,
			&i.CreatedAt,
			&i.SettledAt,
			&i.WithdrawnBy,
			&i.WithdrawnAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getPendingWFHRequestsForSettlement = `-- name: GetPendingWFHRequestsForSettlement :many
SELECT id, member_id, date, status, created_at, settled_at, withdrawn_by, withdrawn_at
FROM wfh_requests
WHERE status = 'pending'
  AND date <= ?
ORDER BY date ASC, created_at ASC
`

func (q *Queries) GetPendingWFHRequestsForSettlement(ctx context.Context, date time.Time) ([]WfhRequest, error) {
	rows, err := q.db.QueryContext(ctx, getPendingWFHRequestsForSettlement, date)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []WfhRequest{}
	for rows.Next() {
		var i WfhRequest
		if err := rows.Scan(
			&i.ID,
			&i.MemberID,
			&i.Date,
			&i.Status,
			&i.CreatedAt,
			&i.SettledAt,
			&i.WithdrawnBy,
			&i.WithdrawnAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getWFHRequestByID = `-- name: GetWFHRequestByID :one
SELECT id, member_id, date, status, created_at, settled_at, withdrawn_by, withdrawn_at
FROM wfh_requests
WHERE id = ?
`

func (q *Queries) GetWFHRequestByID(ctx context.Context, id string) (WfhRequest, error) {
	row := q.db.QueryRowContext(ctx, getWFHRequestByID, id)
	var i WfhRequest
	err := row.Scan(
		&i.ID,
		&i.MemberID,
		&i.Date,
		&i.Status,
		&i.CreatedAt,
		&i.SettledAt,
		&i.WithdrawnBy,
		&i.WithdrawnAt,
	)
	return i, err
}

const getWFHRequestByMemberAndDate = `-- name: GetWFHRequestByMemberAndDate :one
SELECT id, member_id, date, status, created_at, settled_at, withdrawn_by, withdrawn_at
FROM wfh_requests
WHERE member_id = ? AND date = ?
`

type GetWFHRequestByMemberAndDateParams struct {
	MemberID string    `json:"member_id"`
	Date     time.Time `json:"date"`
}

func (q *Queries) GetWFHRequestByMemberAndDate(ctx context.Context, arg GetWFHRequestByMemberAndDateParams) (WfhRequest, error) {
	row := q.db.QueryRowContext(ctx, getWFHRequestByMemberAndDate, arg.MemberID, arg.Date)
	var i WfhRequest
	err := row.Scan(
		&i.ID,
		&i.MemberID,
		&i.Date,
		&i.Status,
		&i.CreatedAt,
		&i.SettledAt,
		&i.WithdrawnBy,
		&i.WithdrawnAt,
	)
	return i, err
}

const getWFHRequestsByDate = `-- name: GetWFHRequestsByDate :many
SELECT id, member_id, date, status, created_at, settled_at, withdrawn_by, withdrawn_at
FROM wfh_requests
WHERE date = ?
ORDER BY created_at ASC
`

func (q *Queries) GetWFHRequestsByDate(ctx context.Context, date time.Time) ([]WfhRequest, error) {
	rows, err := q.db.QueryContext(ctx, getWFHRequestsByDate, date)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []WfhRequest{}
	for rows.Next() {
		var i WfhRequest
		if err := rows.Scan(
			&i.ID,
			&i.MemberID,
			&i.Date,
			&i.Status,
			&i.CreatedAt,
			&i.SettledAt,
			&i.WithdrawnBy,
			&i.WithdrawnAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getWFHRequestsByDateAndStatus = `-- name: GetWFHRequestsByDateAndStatus :many
SELECT id, member_id, date, status, created_at, settled_at, withdrawn_by, withdrawn_at
FROM wfh_requests
WHERE date = ? AND status = ?
ORDER BY created_at ASC
`

type GetWFHRequestsByDateAndStatusParams struct {
	Date   time.Time `json:"date"`
	Status string    `json:"status"`
}

func (q *Queries) GetWFHRequestsByDateAndStatus(ctx context.Context, arg GetWFHRequestsByDateAndStatusParams) ([]WfhRequest, error) {
	rows, err := q.db.QueryContext(ctx, getWFHRequestsByDateAndStatus, arg.Date, arg.Status)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []WfhRequest{}
	for rows.Next() {
		var i WfhRequest
		if err := rows.Scan(
			&i.ID,
			&i.MemberID,
			&i.Date,
			&i.Status,
			&i.CreatedAt,
			&i.SettledAt,
			&i.WithdrawnBy,
			&i.WithdrawnAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getWFHRequestsByMember = `-- name: GetWFHRequestsByMember :many
SELECT id, member_id, date, status, created_at, settled_at, withdrawn_by, withdrawn_at
FROM wfh_requests
WHERE member_id = ?
ORDER BY date DESC
`

func (q *Queries) GetWFHRequestsByMember(ctx context.Context, memberID string) ([]WfhRequest, error) {
	rows, err := q.db.QueryContext(ctx, getWFHRequestsByMember, memberID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []WfhRequest{}
	for rows.Next() {
		var i WfhRequest
		if err := rows.Scan(
			&i.ID,
			&i.MemberID,
			&i.Date,
			&i.Status,
			&i.CreatedAt,
			&i.SettledAt,
			&i.WithdrawnBy,
			&i.WithdrawnAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getWFHRequestsByMemberAndPeriod = `-- name: GetWFHRequestsByMemberAndPeriod :many
SELECT id, member_id, date, status, created_at, settled_at, withdrawn_by, withdrawn_at
FROM wfh_requests
WHERE member_id = ?
  AND date >= ?
  AND date <= ?
  AND status IN ('pending', 'approved')
ORDER BY date ASC
`

type GetWFHRequestsByMemberAndPeriodParams struct {
	MemberID string    `json:"member_id"`
	Date     time.Time `json:"date"`
	Date_2   time.Time `json:"date_2"`
}

func (q *Queries) GetWFHRequestsByMemberAndPeriod(ctx context.Context, arg GetWFHRequestsByMemberAndPeriodParams) ([]WfhRequest, error) {
	rows, err := q.db.QueryContext(ctx, getWFHRequestsByMemberAndPeriod, arg.MemberID, arg.Date, arg.Date_2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []WfhRequest{}
	for rows.Next() {
		var i WfhRequest
		if err := rows.Scan(
			&i.ID,
			&i.MemberID,
			&i.Date,
			&i.Status,
			&i.CreatedAt,
			&i.SettledAt,
			&i.WithdrawnBy,
			&i.WithdrawnAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const updateWFHRequestStatus = `-- name: UpdateWFHRequestStatus :execresult
UPDATE wfh_requests
SET status = ?, settled_at = ?
WHERE id = ?
`

type UpdateWFHRequestStatusParams struct {
	Status    string       `json:"status"`
	SettledAt sql.NullTime `json:"settled_at"`
	ID        string       `json:"id"`
}

func (q *Queries) UpdateWFHRequestStatus(ctx context.Context, arg UpdateWFHRequestStatusParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, updateWFHRequestStatus, arg.Status, arg.SettledAt, arg.ID)
}

const updateWFHRequestWithdrawn = `-- name: UpdateWFHRequestWithdrawn :execresult
UPDATE wfh_requests
SET status = 'withdrawn', withdrawn_by = ?, withdrawn_at = CURRENT_TIMESTAMP
WHERE id = ?
`

type UpdateWFHRequestWithdrawnParams struct {
	WithdrawnBy sql.NullString `json:"withdrawn_by"`
	ID          string         `json:"id"`
}

func (q *Queries) UpdateWFHRequestWithdrawn(ctx context.Context, arg UpdateWFHRequestWithdrawnParams) (sql.Result, error) {
	return q.db.ExecContext(ctx, updateWFHRequestWithdrawn, arg.WithdrawnBy, arg.ID)
}
