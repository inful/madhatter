...
func TestEngine_MultipleSeparateLeaves(t *testing.T) {
	...
	leave3ID, err := db.CreateLeaveRecord(ctx, bobID, "2024-01-23", "2024-01-23", database.LeaveTypeLeave)
	require.NoError(t, err)
...
func TestEngine_CoverRotationAcrossMultipleMembers(t *testing.T) {
	...
	leaveAlice1, err := db.CreateLeaveRecord(ctx, aliceID, "2024-01-15", "2024-01-15", database.LeaveTypeLeave)
	require.NoError(t, err)
	err = engine.AssignCoversForLeave(ctx, leaveAlice1)
	require.NoError(t, err)

	// Bob takes leave on Tue Jan 16 (his day)
	leaveBob1, err := db.CreateLeaveRecord(ctx, bobID, "2024-01-16", "2024-01-16", database.LeaveTypeLeave)
	require.NoError(t, err)
	err = engine.AssignCoversForLeave(ctx, leaveBob1)
	require.NoError(t, err)

	// Charlie takes leave on Wed Jan 17 (his day)
	leaveCharlie1, err := db.CreateLeaveRecord(ctx, charlieID, "2024-01-17", "2024-01-17", database.LeaveTypeLeave)
	require.NoError(t, err)
	err = engine.AssignCoversForLeave(ctx, leaveCharlie1)
	require.NoError(t, err)

	// Alice takes leave again on Mon Jan 22
	leaveAlice2, err := db.CreateLeaveRecord(ctx, aliceID, "2024-01-22", "2024-01-22", database.LeaveTypeLeave)
	require.NoError(t, err)
