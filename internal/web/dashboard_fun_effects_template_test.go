...
func renderDashboardForFunEffects(t *testing.T, funEffectsConfetti, funEffectsSnow bool) string {
	t.Helper()
...
func TestDashboard_FunEffects_ConfigDivAndScripts(t *testing.T) {
	body := renderDashboardForFunEffects(t, true, true)
...
// case: when both flags are false (CONFETTI_ENABLED=false in env
// AND today is not full-team-and-business-day, or just today is
// not a full-team day), the config div carries false and the
// scripts are still loaded. The script tags ship unconditionally
// because the disabled-effect path is data-driven, not script-
// loading-driven — saves a round-trip and keeps the CSP simple.
func TestDashboard_FunEffects_FalseFlagsRenderFalse(t *testing.T) {
	body := renderDashboardForFunEffects(t, false, false)
	...
	// The script tags still load even when both flags are false
	// — fun-effects.js no-ops on data-confetti="false" /
	// data-snow="false". This avoids the page re-fetching the
	// vendored bundle on the rare day the gate flips.
	assert.Contains(t, body, `src="/static/js/canvas-confetti.browser.min.js"`,
...
// not December, so only data-confetti is true. The snow attribute
// stays false, and fun-effects.js will skip the snow storm.
func TestDashboard_FunEffects_OnlyConfettiTrue(t *testing.T) {
	body := renderDashboardForFunEffects(t, true, false)
...
func TestDashboard_FunEffects_OnlySnowTrue(t *testing.T) {
	body := renderDashboardForFunEffects(t, false, true)
